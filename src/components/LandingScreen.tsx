...
import { ChevronRight, Link2, Check, Share2 } from 'lucide-react';
...
export const LandingScreen: React.FC = () => {
  ...
  const [nativeShareAvail] = useState(() => typeof navigator !== 'undefined' && !!navigator.share);
  ...
  const handleNativeShare = async () => {
    try {
      await navigator.share({ title: 'LetterRush', text: shareText, url: shareUrl });
    } catch {
    ...
    <div className="screen landing-screen">
      ...
      <div className="landing-share-footer">
        ...
        <div style={{ display: 'flex', gap: '0.6rem', alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap' }}>
          {/* Platform buttons */}
          {PLATFORMS.map(({ id, label, color, getUrl, Icon }) => (
            <button
              key={id}
              aria-label={label}
              title={label}
              onClick={() => window.open(getUrl(shareUrl, shareText), '_blank', 'noopener,noreferrer')}
              style={{
                width: 48,
                height: 48,
                borderRadius: 14,
          ...
          <button
            ...
            style={{
              ...
              height: 48,
              borderRadius: 14,
              background: copied ? 'var(--success, #22c55e)' : 'var(--card)',
        ...
        <div style={{ marginTop: '1rem', color: 'var(--text-muted)', fontSize: '0.72rem', opacity: 0.55, textAlign: 'center' }}>
