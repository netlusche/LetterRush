...
export const ShareFooter: React.FC = () => {
  ...
  const handleNativeShare = async () => {
    try { await navigator.share({ title: 'LetterRush', text: shareText, url: shareUrl }); }
    catch { /* cancelled */ }
  };
  ...
  return (
    <div className="landing-share-footer">
      <p style={{ margin: '0 0 0.65rem 0', fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: 600, letterSpacing: '0.04em', textTransform: 'uppercase' }}>
        {t.shareGame}
      </p>
      ...
      <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center', justifyContent: 'center', flexWrap: 'nowrap' }}>
        ...
        {/* Facebook */}
        <button aria-label="Facebook" title="Facebook"
          style={{ ...btn, background: 'linear-gradient(180deg, #1877F2 0%, #0052CC 100%)' }}
        ...
        {/* Telegram */}
        <button aria-label="Telegram" title="Telegram"
          style={{ ...btn, background: 'linear-gradient(180deg, #2AABEE 0%, #0088CC 100%)' }}
          onClick={() => open(`https://t.me/share/url?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(shareText)}`)}
          onMouseEnter={hover} onMouseLeave={unhover}>
          <TelegramIcon />
        </button>

        {/* Reddit */}
        <button aria-label="Reddit" title="Reddit"
          style={{ ...btn, background: '#FF4500' }}
          onClick={() => open(`https://www.reddit.com/submit?url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(shareText)}`)}
          onMouseEnter={hover} onMouseLeave={unhover}>
          <RedditIcon />
        </button>

        {/* Native share — only on browsers that support it */}
        {nativeShareAvailable && (
          <button aria-label={t.shareGame} title={t.shareGame}
        ...
        <button
          ...
          onClick={handleCopy}
          onMouseEnter={hover} onMouseLeave={unhover}>
          {copied ? <Check size={22} color="white" /> : <Copy size={20} color="rgba(255,255,255,0.7)" />}
        </button>
